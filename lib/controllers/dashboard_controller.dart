...
// Get water budget from Firebase
Future<double?> getWaterBudgetLiters() async {
  final u = _auth.currentUser;
  if (u == null) return null;
  final doc = await _fs.collection('users').doc(u.uid).get();
  return _safeToDouble(doc.data()?['waterBudgetLiters']);
}

// Set water budget in Firebase
Future<void> setWaterBudgetLiters(double liters) async {
  final u = _auth.currentUser;
  if (u == null) return;
  await _fs.collection('users').doc(u.uid).set({
    'waterBudgetLiters': liters,
  }, SetOptions(merge: true));
}
